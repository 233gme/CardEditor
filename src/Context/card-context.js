...
const CardProvider = (props) => {
  ...
  const saveNewChanges = (value) => {
    ...
    setCards({ cards: newCardState });
  }

  const deleteCardHandler = () => {
    setCards({ cards: [...cards].filter(item => !item.chooseСardFlag) });
  }

  const createCardHandler = () => {
    setCards({ cards: [...cards].concat({ id: uuidv4(), title: '', text: '', editModeFlag: false, chooseСardFlag: false }), });
  }
  ...
  return (
    <CardContext.Provider value={{
      ...
      onView: viewCardHandler,
      onSave: saveNewChanges,
      onDelete: deleteCardHandler,
      onAdd: createCardHandler
    }}>
      {props.children}
